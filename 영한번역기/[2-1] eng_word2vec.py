...
with open("eng_dictionary.bin", "rb") as kd:
...
with tf.Session(graph=graph) as session:
    ...
    num_step = 100000 
    average_loss = 0
    with open('eng_indexed.bin', 'rb') as engids:
        ids = pickle.load(engids)
        while step <= num_step:
...
with open("eng_embedding.bin", "wb") as eng_embedding:
    pickle.dump(final_embeddings, eng_embedding)
...
r_dict = open("eng_reverse_dictionary.bin", "rb")
